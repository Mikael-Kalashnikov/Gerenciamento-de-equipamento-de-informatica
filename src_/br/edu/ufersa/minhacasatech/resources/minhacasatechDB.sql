...
CREATE VIEW public.vendas_view AS
 SELECT venda.id,
   ...
   FROM ((public.venda
     JOIN public.cliente ON ((venda.id_cliente = cliente.id)))
     JOIN public.funcionario ON ((venda.id_funcionario = funcionario.id)));
...
COPY public.cliente (id, nome, cpf, telefone, data_cadastro) FROM stdin;
1	João Pedro	018.447.456-21	(84)99145-3210	2023-10-17
2	Caio Anderson	124.760.812-47	(84)99145-3210	2023-10-17
\.
...
COPY public.equipamento (id, nome, quantidade, preco, num_serie, data_cadastro, id_local) FROM stdin;
1	Mouse	74	30.50	PQRS84PQRS84745	2023-10-17	1
2	Teclado	74	32.20	PQRS84PQRS84744	2023-10-17	1
3	Headset	22	90.00	PQRS84PQRS84743	2023-10-17	2
\.
...
COPY public.funcionario (id, is_responsavel, nome, cpf, login, senha, telefone, endereco, data_cadastro) FROM stdin;
1	t	Kanalense	017.246.785-71	kanalense	kanalense123	(84)99132-9467	Avenida Rio Branco, 400	2023-10-17
2	t	Toinho	489.456.174-94	toinho	toinho123	(84)99176-3320	Avenida Rio Branco, 444	2023-10-17
3	f	Marcos	462.761.842-32	marcao	marcao123	(84)99174-3320	Rua dos Lobos, 0	2023-10-17
\.
...
1	Casa do Kanalaense	Depósito	2023-10-17
...
COPY public.venda (id, status, valor_total, data_cadastro, data_venda, id_cliente, id_funcionario) FROM stdin;
1	Em Andamento	\N	2023-10-17	\N	1	1
2	Em Andamento	\N	2023-10-17	\N	2	1
\.
...
SELECT pg_catalog.setval('public.equipamento_id_seq', 3, true);
...
SELECT pg_catalog.setval('public.local_id_seq', 2, true);
